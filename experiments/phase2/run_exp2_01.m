...
%
% 출력:
%   results - 3D 결과 구조체 [n_scenarios, n_schemes, n_metrics]
    ...
    fprintf('[실험 설정]\n');
    fprintf('  시나리오: %d개 (Low/Mid/High)\n', n_scenarios);
    fprintf('  스킴: %d개 (Baseline + 제안 3개)\n', n_schemes);
    fprintf('  반복 횟수: %d\n', n_runs);
    ...
    for s = 1:n_scenarios
        ...
        for scheme_idx = 1:n_schemes
            ...
            for run = 1:n_runs
                ...
                if mod(run, max(1, floor(n_runs/3))) == 1
                ...
                try
                    ...
                    if mod(run, max(1, floor(n_runs/3))) == 0
                        fprintf('완료\n');
                    end
